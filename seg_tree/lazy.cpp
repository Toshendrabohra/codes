...
ll query(vector<ll>&a , vector<ll> &seg,vector<ll> &lazy,ll s,ll e,ll indx,ll qindx){
if(lazy[indx]!=0)
{//seg[indx]+=lazy[indx];
	if(s!=e)
	{
...
ll ans=0;
	...
	{
	ll mid=(s+e)/2;
	if(qindx<=mid && qindx>=s)
	ans= query(a,seg,lazy,s,mid,2*indx,qindx);
	else if(qindx<=e && qindx>mid)
	ans= query(a,seg,lazy,mid+1,e,2*indx+1,qindx);
	}
//	seg[indx]=seg[2*indx]+seg[2*indx+1];
	return ans;
}
...
f(i,q)
{
	...
	if(x==1)
	{cin>>u>>v>>que;
	update(a,seg,lazy,0,n-1,1,u-1,v-1,que);
	}
	else
	{
		cin>>u;
	o(query(a,seg,lazy,0,n-1,1,u-1));en;
	}
}
